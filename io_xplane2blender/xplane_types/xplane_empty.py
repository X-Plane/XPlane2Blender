...
class XPlaneEmpty(XPlaneObject):
    ...
    def collect(self):
        ...
        if special_empty_props.special_type == EMPTY_USAGE_MAGNET:
            ...
            if not magnet_props.debug_name:
                logger.error("Empty '{}' must have a non-blank Debug Name".format(self.blenderObject.name))
            if (not magnet_props.magnet_type_is_xpad and
                not magnet_props.magnet_type_is_flashlight):
                logger.error("Magnet {debug_name} must have 'xpad' and/or 'flashlight'"
                             .format(debug_name=magnet_props.debug_name))
    ...
    def write(self):
        ...
        if (int(bpy.context.scene.xplane.version) >= 1130 and
            ...
            o += '\n'
        elif (int(bpy.context.scene.xplane.version) >= 1130 and
              special_empty_props.special_type == EMPTY_USAGE_MAGNET):
            bake_matrix = self.xplaneBone.getBakeMatrixForAttached()
            em_location = xplane_helpers.vec_b_to_x(bake_matrix.to_translation())
            #yaw,pitch,roll
            theta, psi, phi = tuple(map(math.degrees,bake_matrix.to_euler()[:]))
