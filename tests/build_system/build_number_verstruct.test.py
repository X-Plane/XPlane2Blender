...
class TestBuildNumberVerStruct(XPlaneTestCase):
    ...
    def test_constructor_defaults_correct(self):
        ...
        self.assertEqual(ver_s.addon_version,      (0,0,0), "addon_version %s does not match it's default %s" % (ver_s.addon_version, (0,0,0)))
        self.assertEqual(ver_s.build_type,         xplane_constants.BUILD_TYPE_DEV, "build_type %s does not match it's default %s" % (ver_s.build_type, xplane_constants.BUILD_TYPE_DEV))
        self.assertEqual(ver_s.build_type_version, 0, "build_type_version %s does not match it's default %s" % (ver_s.build_type_version,0))
        self.assertEqual(ver_s.data_model_version, 0, "data_model_version %s does not match it's default %s" % (ver_s.data_model_version,0))
        self.assertEqual(ver_s.build_number,       xplane_constants.BUILD_NUMBER_NONE,"build_number %s does not match it's default %s" % (ver_s.build_number,xplane_constants.BUILD_NUMBER_NONE))
    ...
    def test_invalid_ver_addition(self):
        ...
        self.assertEqual(orig_history_len, len(self.history), "History was not returned to original length after invalid addition")

    def test_valid_ver_addition(self):
        orig_history_len = len(self.history)
        new_entry = VerStruct.add_to_version_history(self.current)
        self.assertIsNotNone(new_entry, "Valid entry was not allowed into version history")
        self.assertNotEqual(orig_history_len, orig_history_len+1, "History length was not incremented by one after valid addition")
    ...
    def test_parse_version(self):
        ...
        for test_v, test_v_res in correct_versions_legacy:
            ...
            self.assertIsNotNone(v_res, "VerStruct.parse_version did not allow valid legacy style version %s through" % test_v)
            self.assertEqual(test_v_res, v_res, "Test string %s did not parse to expected data %s" % (test_v,str(v_res)))
        ...
        for test_v, test_v_res in correct_versions_modern:
            v_res = VerStruct.parse_version(test_v)
            self.assertIsNotNone(v_res, "VerStruct.parse_version did not allow valid modern style version %s through" % test_v)
            self.assertEqual(test_v_res, v_res, "Test string %s did not parse to expected data %s" % (test_v,str(v_res)))
    ...
    def test_rich_compare(self):
        ...
        self.assertEqual(legacy, legacy_copy, "VerStruct.__eq__ not implemented correctly")
        self.assertNotEqual(legacy, beta_4, "VerStruct.__ne__ not implemented correctly")
